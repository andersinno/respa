...
- `DEBUG_TOOLBAR`: Whether to enable Django Debug Toolbar. NOTE: The `DEBUG` setting has to be `True` as well. [Django Debug Toolbar](https://django-debug-toolbar.readthedocs.io/en/latest/index.html).
...
- `INTERNAL_IPS`: Django INTERNAL_IPS setting allows some debugging aids for the addresses specified here. [Django setting](https://docs.djangoproject.com/en/2.2/ref/settings/#internal-ips). Example value `'127.0.0.1'`. NOTE: If `DEBUG_TOOLBAR` is enabled, make sure this is set correctly as well.
...
client secret in the production Tunnistamo or modifying tamusers to use
...
http://localhost:8000/accounts/tampere/login/callback/
