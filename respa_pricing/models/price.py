...
class PriceList(models.Model):
    ...
    @classmethod
    def get_price_info(cls, product, user_group, event_type, begin, end):
        """
        The source of the price should be so that the highest amount is
        always chosen. E.g. If price for user group is 20 and for event_
        type it's 10, the use the price for user group as (20 > 10).
        However, there is an exception that if selected user group or
        event_type's price is 0, that should be used for price source.
        """
        price_list = PricedProduct.objects.get(product=product).price_list
        user_group_item = price_list.usergroup_prices.filter(user_group__id=user_group).first()
        total_price = price_by_user_group = user_group_item.get_price_for_reservation(begin, end)
        event_item = price_list.event_prices.filter(event__id=event_type).first()
        price_source = user_group_item

        if event_item and total_price != 0:
            price_by_event = event_item.get_price_for_reservation(begin, end)
            if (price_by_event >= price_by_user_group) or price_by_event == 0:
                price_source = event_item
                total_price = price_by_event

        price_details = price_source.get_price_details()
        price_details['total_price'] = str(total_price)
