...
class DaysForm(forms.ModelForm):
    opens = forms.TimeField(
        ...
        widget=forms.TimeInput(
            format="%H:%M",
            attrs={"class": "text-input form-control", "placeholder": "hh:mm"},
        ),
    )
...
class ResourceForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "min_period": forms.Select(choices=(thirty_minute_increment_choices)),
...
class PeriodFormset(forms.BaseInlineFormSet):
    def _get_days_formset(self, form, extra_days=1):
        days_formset = inlineformset_factory(
            Period, Day, form=DaysForm, extra=extra_days, validate_max=True
        )

        return days_formset(
            instance=form.instance,
            data=form.data if form.is_bound else None,
            prefix="days-%s" % (form.prefix,),
        )
...
def get_translated_field_count(image_formset=None):
    ...
    if translated_fields:
        for key, value in LANGUAGES:
            postfix = "_" + key
            images_fields_count = _get_images_formset_translated_fields(
                image_formset, postfix
            )
            lang_num[key] = (
                sum(x.endswith(postfix) for x in translated_fields)
                + images_fields_count
            )
...
def _get_images_formset_translated_fields(images_formset, lang_postfix):
    ...
    for form in image_forms:
        if form.initial:
            images_translation_count += len(
                [x for x in form.initial if x.endswith(lang_postfix)]
            )
...
class UnitAuthorizationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            user_has_unit_auth = (
                self.request.user.unit_authorizations.to_unit(unit)
                .admin_level()
                .exists()
            )
            user_has_unit_group_auth = (
                self.request.user.unit_group_authorizations.to_unit(unit)
                .admin_level()
                .exists()
            )
    ...
    def clean(self):
        ...
        user_has_unit_auth = (
            self.request.user.unit_authorizations.to_unit(unit).admin_level().exists()
        )
        user_has_unit_group_auth = (
            self.request.user.unit_group_authorizations.to_unit(unit)
            .admin_level()
...
def get_unit_authorization_formset(request=None, extra=1, instance=None):
    ...
    if request.method == "GET":
        ...
        return unit_authorization_formset(
            request=request, data=request.POST, instance=instance
        )
