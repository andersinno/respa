...
def space_resource(space_resource_type):
    return Resource.objects.create(
        type=space_resource_type, authentication="none", name="resource"
    )
...
def exceptional_period(resource_with_opening_hours):
    ...
    date = period.start
    Day.objects.create(period=period, weekday=date.weekday(), closed=True)
    date = date + datetime.timedelta(days=1)
    Day.objects.create(
        period=period, weekday=date.weekday(), opens="12:00", closes="13:00"
    )
    date = date + datetime.timedelta(days=1)
    Day.objects.create(period=period, weekday=date.weekday(), closed=True)
...
def equipment_category():
    return EquipmentCategory.objects.create(name="test equipment category")
...
def equipment_alias(equipment):
    equipment_alias = EquipmentAlias.objects.create(
...
def resource_group(resource_in_unit):
    group = ResourceGroup.objects.create(
        identifier="test_group", name="Test resource group"
    )
...
def resource_group2(resource_in_unit2):
    group = ResourceGroup.objects.create(
        identifier="test_group_2", name="Test resource group 2"
    )
...
def test_municipality():
    municipality = Municipality.objects.create(id="foo", name="Foo")
    return municipality
...
@pytest.fixture
def resource_with_accessibility_data2(
    resource_in_unit2,
    accessibility_viewpoint_wheelchair,
    accessibility_viewpoint_hearing,
    accessibility_value_green,
...
@pytest.fixture
def resource_with_accessibility_data3(
    resource_in_unit3,
    accessibility_viewpoint_wheelchair,
    accessibility_viewpoint_hearing,
    accessibility_value_green,
