...
DEFAULT_RESERVATION_EXTRA_FIELDS = (
    "reserver_name",
...
DEFAULT_REQUIRED_RESERVATION_EXTRA_FIELDS = (
    "reserver_name",
    "reserver_phone_number",
    "reserver_address_street",
    "reserver_address_zip",
...
def test_disallowed_methods(all_user_types_api_client, list_url):
    ...
    """
    check_disallowed_methods(
        all_user_types_api_client, (list_url,), ("put", "patch", "delete")
    )


@pytest.mark.django_db
def test_reservation_requires_authenticated_user(
...
@pytest.mark.django_db
def test_authenticated_user_can_make_reservation(
    ...
    response = api_client.post(list_url, data=reservation_data)
    assert response.status_code == 201, "Request failed with: %s" % (
...
@pytest.mark.django_db
def test_authenticated_user_can_delete_reservation(
...
@pytest.mark.django_db
def test_reservation_limit_per_user(
    ...
    # the user already has one reservation, making another reservation should not be
    # possible as the active reservation limit is one
    response = api_client.post(
        list_url, data=reservation_data, HTTP_ACCEPT_LANGUAGE="en"
    )

    assert response.status_code == 400
    assert_non_field_errors_contain(
        response, "Maximum number of active reservations for this resource exceeded."
    )


@pytest.mark.django_db
def test_old_reservations_are_excluded(
    ...
    # making another reservation should be possible because the other reservation is
    # in the past.
    response = api_client.post(
        list_url, data=reservation_data, HTTP_ACCEPT_LANGUAGE="en"
    )
...
@pytest.mark.django_db
def test_opening_hours(api_client, list_url, reservation_data, resource_group, user):
    """
    ...
    assert response.status_code == 400
    assert_non_field_errors_contain(
        response, "You must start and end the reservation during opening hours"
    )
...
@pytest.mark.django_db
def test_normal_user_cannot_make_reservation_longer_than_max_period(
    api_client, list_url, reservation_data, user
):
    """
    Tests that a normal user cannot make reservations longer than the resource's
    max period.
    """
    api_client.force_authenticate(user=user)
...
def test_admin_can_make_reservation_outside_open_hours(
    ...
    # begin time before opening time, end time after closing time, longer than max
    # period 2h
    reservation_data["begin"] = "2115-04-04T05:00:00+02:00"
    reservation_data["end"] = "2115-04-04T21:00:00+02:00"
    response = api_client.post(
...
def test_comments_are_only_for_admins(
    api_client, list_url, reservation_data, user, staff_user, general_admin
):
    api_client.force_authenticate(user=user)
...
def test_user_data_correct_and_only_for_admins(
    ...
    """
    Tests that user object is returned within Reservation data and it is in the correct
    form.
...
@pytest.mark.django_db
def test_reservation_can_be_modified_by_overlapping_reservation(
    api_client, reservation, reservation_data, user
):
    """
    Tests that a reservation can be modified with times that overlap with the original
    times.
    """
    api_client.force_authenticate(user=user)
    detail_url = reverse("reservation-detail", kwargs={"pk": reservation.pk})
...
def test_non_reservable_resource_restrictions(
    ...
    Creating a new reservation with POST and updating an existing one with PUT are both
    tested.
    """
    ...
    staff_reservation_data["begin"] = dateparse.parse_datetime(
        "2115-04-08T09:00:00+02:00"
    )
    staff_reservation_data["end"] = dateparse.parse_datetime(
        "2115-04-08T10:00:00+02:00"
    )
    detail_url = reverse(
        "reservation-detail", kwargs={"pk": staff_reservation_data["id"]}
    )
...
def test_admins_can_make_reservations_for_others(
    ...
    """
    Tests that a staff member can make reservations for other people without normal
    user restrictions.
    """
    api_client.force_authenticate(user=general_admin)
    ...
    # create a new reservation, which is also too long, outside the opening hours and
    # exceeds normal user reservation limit. creating such a reservation for a normal
    # user should be possible for a staff member
    reservation_data["begin"] = dateparse.parse_datetime("2115-04-04T13:00:00+02:00")
...
@pytest.mark.django_db
def test_reservation_user_filter(
...
@pytest.mark.django_db
def test_reservation_time_filters(
    api_client, list_url, reservation, resource_in_unit, user
):
    reservation2 = Reservation.objects.create(
        resource=resource_in_unit,
    ...
    assert {reservation.id, reservation2.id}.issubset(
        set(res["id"] for res in response.data["results"])
    )
    ...
    response = api_client.get(
        list_url
        + "?start=2065-04-06T11:00:00%2b02:00"
        + "&end=2065-04-06T12:00:00%2b02:00"
    )
    assert response.data["count"] == 0
    response = api_client.get(
        list_url
        + "?start=2005-04-07T11:30:00%2b02:00"
        + "&end=2115-04-04T09:30:00%2b02:00"
    )
    assert response.data["count"] == 2
    assert {reservation.id, reservation2.id}.issubset(
        set(res["id"] for res in response.data["results"])
...
@pytest.mark.parametrize(
    "input_hours,input_mins,expected",
    [
...
def test_max_reservation_period_error_message(
    ...
    resource_in_unit.max_period = datetime.timedelta(
        hours=input_hours, minutes=input_mins
    )
    resource_in_unit.save()
    ...
    assert (
        ...
        == "The maximum reservation length is %s" % expected
    )
...
@pytest.mark.parametrize(
    ...
    [(False, Reservation.CONFIRMED), (True, Reservation.REQUESTED)],
)
@pytest.mark.django_db
def test_state_on_new_reservations(
    user_api_client,
    list_url,
    ...
    if need_manual_confirmation:
        resource_in_unit.reservation_metadata_set = ReservationMetadataSet.objects.get(
            name="default"
        )
    resource_in_unit.save()
...
@pytest.mark.django_db
def test_illegal_state_set(
    user_api_client, list_url, detail_url, reservation_data, state
):
...
@pytest.mark.parametrize("need_manual_confirmation", [False, True])
@pytest.mark.django_db
def test_extra_fields_visibility(
    user_api_client,
    list_url,
    detail_url,
    reservation,
...
def test_extra_fields_visibility_per_user(
    ...
    if has_perm:
        assign_perm(
            "unit:can_view_reservation_extra_fields", test_user, resource_in_unit.unit
        )
...
@pytest.mark.django_db
def test_extra_fields_required_for_paid_reservations(
    user_api_client,
    staff_api_client,
    staff_user,
    list_url,
...
@pytest.mark.django_db
def test_staff_event_restrictions(
    user_api_client,
    staff_api_client,
    staff_user,
    list_url,
...
@pytest.mark.django_db
def test_extra_fields_can_be_set_for_paid_reservations(
    user_api_client, list_url, reservation_data_extra, resource_in_unit
):
    ...
    response = user_api_client.put(
        "%s%s/" % (list_url, reservation.pk), data=reservation_data_extra
    )
    assert response.status_code == 200
    reservation.refresh_from_db()
...
@pytest.mark.django_db
def test_extra_fields_ignored_for_non_paid_reservations(
    user_api_client, list_url, reservation_data_extra, resource_in_unit
):
    response = user_api_client.post(list_url, data=reservation_data_extra)
    assert response.status_code == 201
...
@pytest.mark.django_db
def test_user_can_see_her_reservations_in_all_states(
    user_api_client, list_url, reservations_in_all_states
):
...
@pytest.mark.django_db
def test_user_cannot_see_others_denied_or_cancelled_reservations(
    api_client, user2, list_url, reservations_in_all_states
):
    api_client.force_authenticate(user=user2)
    response = api_client.get(list_url)
    assert response.status_code == 200
...
def test_reservation_can_be_confirmed_with_permission(
    api_client, general_admin, detail_url, reservation, reservation_data
):
    reservation.state = Reservation.REQUESTED
    reservation.save()
    reservation_data["state"] = Reservation.CONFIRMED
    assign_perm(
        "unit:can_approve_reservation", general_admin, reservation.resource.unit
    )
    api_client.force_authenticate(user=general_admin)
    response = api_client.put(detail_url, data=reservation_data)
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_user_cannot_modify_or_cancel_manually_confirmed_reservation(
...
@pytest.mark.django_db
def test_denied_and_cancelled_reservations_not_active(
    user_api_client, reservation, reservation_data, list_url, resource_in_unit, state
):
...
@pytest.mark.django_db
def test_cannot_make_reservation_in_the_past(
    user_api_client, reservation_data, list_url
):
    reservation_data.update(
        begin="2010-04-04T11:00:00+02:00", end="2010-04-04T12:00:00+02:00"
    )
...
@pytest.mark.django_db
def test_need_manual_confirmation_filter(
    user_api_client, user, list_url, reservation, other_resource
):
    other_resource.need_manual_confirmation = True
    other_resource.reservation_metadata_set = ReservationMetadataSet.objects.get(
        name="default"
    )
    other_resource.save()
    ...
    response = user_api_client.get(
        "%s%s" % (list_url, "?need_manual_confirmation=false")
    )
    assert response.status_code == 200
    reservation_ids = set([res["id"] for res in response.data["results"]])
    ...
    response = user_api_client.get(
        "%s%s" % (list_url, "?need_manual_confirmation=true")
    )
    assert response.status_code == 200
    reservation_ids = set([res["id"] for res in response.data["results"]])
...
def test_reservation_mails(
    ...
    response = user_api_client.post(
        list_url, data=reservation_data_extra, format="json"
    )
    assert response.status_code == 201
...
@override_settings(RESPA_MAILS_ENABLED=True)
@pytest.mark.django_db
def test_reservation_created_mail(
    user_api_client, list_url, reservation_data, user, reservation_created_notification
):
    response = user_api_client.post(list_url, data=reservation_data, format="json")
...
@override_settings(RESPA_MAILS_ENABLED=True)
@pytest.mark.django_db
def test_reservation_html_mail(
    user_api_client, list_url, reservation_data, user, reservation_created_notification
):
...
@override_settings(RESPA_MAILS_ENABLED=True)
@pytest.mark.django_db
def test_reservation_mail_empty_text_body_should_become_html_body_without_tags(
    user_api_client, list_url, reservation_data, user, reservation_created_notification
):
    with switch_language(reservation_created_notification, "en"):
...
@override_settings(RESPA_MAILS_ENABLED=True)
@pytest.mark.django_db
def test_reservation_mail_can_have_subject_only(
    user_api_client, list_url, reservation_data, user, reservation_created_notification
):
    with switch_language(reservation_created_notification, "en"):
        reservation_created_notification.body = ""
...
@override_settings(
    RESPA_MAILS_ENABLED=True, RESPA_IMAGE_BASE_URL="https://foo.bar/baz/"
)
@pytest.mark.django_db
def test_reservation_mail_images(
    user_api_client,
    user,
    list_url,
    reservation_data,
    ...
    with switch_language(reservation_created_notification, "en"):
        reservation_created_notification.body = (
            "image url: {{ resource_main_image_url }}"
        )
        reservation_created_notification.html_body = (
            'image: <img src="{{ resource_ground_plan_image_url }}">'
        )
        reservation_created_notification.save()
    ...
    last_ground_plan_image = ResourceImage.objects.create(
        resource=resource_in_unit, type="ground_plan"
    )
    ...
    check_received_mail_exists(
        ...
        html_body='image: <img src="https://foo.bar/baz/resource_image/{}">'.format(
            last_ground_plan_image.id
        ),
    )
...
@pytest.mark.parametrize("perm_type", ["unit", "resource_group"])
@pytest.mark.django_db
def test_can_approve_filter(
    staff_api_client, staff_user, list_url, reservation, perm_type
):
    reservation.resource.need_manual_confirmation = True
    reservation.resource.reservation_metadata_set = ReservationMetadataSet.objects.get(
        name="default"
    )
    reservation.resource.save()
    reservation.state = Reservation.REQUESTED
...
@pytest.mark.django_db
def test_access_code_cannot_be_set_if_type_none(
    user_api_client, list_url, resource_in_unit, reservation_data
):
    ...
    assert (
        ...
        in response.data["access_code"]
    )


@pytest.mark.django_db
def test_invalid_pin6_access_code(
    user_api_client, list_url, resource_in_unit, reservation_data
):
...
@pytest.mark.django_db
def test_pin6_access_code_is_generated_if_not_set(
    user_api_client, list_url, resource_in_unit, reservation_data
):
    resource_in_unit.access_code_type = Resource.ACCESS_CODE_TYPE_PIN6
    resource_in_unit.save()
...
@pytest.mark.django_db
def test_pin6_access_code_can_be_set(
    user_api_client, list_url, resource_in_unit, reservation_data
):
    resource_in_unit.access_code_type = Resource.ACCESS_CODE_TYPE_PIN6
    resource_in_unit.save()
...
@pytest.mark.django_db
def test_pin4_access_code_is_not_generated(
    user_api_client, list_url, resource_in_unit, reservation_data
):
...
@pytest.mark.django_db
def test_pin6_access_code_cannot_be_modified(
    user_api_client, resource_in_unit, reservation, reservation_data
):
    resource_in_unit.access_code_type = Resource.ACCESS_CODE_TYPE_PIN6
    resource_in_unit.save()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            True,
        ),  # someone else's reservation but having the permission
...
def test_access_code_visibility(
    ...
    if has_perm:
        assign_perm(
            "unit:can_view_reservation_access_code", current_user, resource_in_unit.unit
        )
...
@override_settings(RESPA_MAILS_ENABLED=True)
@pytest.mark.django_db
def test_reservation_created_with_access_code_mail(
    user_api_client, user, resource_in_unit, list_url, reservation_data
):
...
@pytest.mark.django_db
def test_reservation_reservable_before(
    user_api_client, resource_in_unit, list_url, reservation_data
):
    resource_in_unit.reservable_max_days_in_advance = 10
    resource_in_unit.save()

    reservation_data["begin"] = timezone.now().replace(
        hour=12, minute=0, second=0
    ) + datetime.timedelta(days=11)
    reservation_data["end"] = timezone.now().replace(
...
@freeze_time("2115-04-02")
@pytest.mark.django_db
def test_reservation_reservable_after(
    user_api_client, resource_in_unit, list_url, reservation_data
):
    ...
    response = user_api_client.post(list_url, data=reservation_data)
    msg = 'expected status_code {}, received {} with message "{}"'
    assert response.status_code == 400, msg.format(
        400, response.status_code, response.data
    )
...
def test_admins_can_make_reservations_despite_delay(
    ...
    """
    Admin should be able to make reservations regardless of reservation delay
    limitations
    """
    api_client.force_authenticate(user=general_admin)
...
@pytest.mark.django_db
def test_reservation_metadata_set(
...
def test_detail_endpoint_does_not_need_all_true_filter(
    ...
    detail_url = reverse(
        "reservation-detail", kwargs={"pk": reservation_in_the_past.pk}
    )
...
def test_user_permissions_field(
    api_client, user_api_client, user, user2, resource_in_unit, reservation, detail_url
):
    response = api_client.get(detail_url)
    assert response.status_code == 200
    assert response.data["user_permissions"] == {
        "can_delete": False,
        "can_modify": False,
    }
...
def test_charfield_filters(
    ...
    # having the right permission allows the user to filter the other user's
    # reservation also
    assign_perm(
        "unit:can_view_reservation_extra_fields", user, reservation3.resource.unit
    )
    response = user_api_client.get(url_with_filters)
    assert response.status_code == 200
    assert_response_objects(response, (reservation2, reservation3))
...
def test_resource_name_filter(
    user_api_client, user, reservation, reservation2, reservation3, list_url
):
    response = user_api_client.get(list_url)
    assert response.status_code == 200
    assert_response_objects(response, (reservation, reservation2, reservation3))
...
@pytest.mark.django_db
def test_is_favorite_resource_filter(
    user_api_client,
    user,
    resource_in_unit,
    reservation,
...
@pytest.mark.django_db
def test_resource_group_filter(
    user_api_client,
    user,
    reservation,
    reservation2,
    ...
    group_1 = ResourceGroup.objects.create(
        name="test group 1", identifier="test_group_1"
    )
    resource_in_unit.groups.set([group_1])

    group_2 = ResourceGroup.objects.create(
        name="test group 2", identifier="test_group_2"
    )
    resource_in_unit2.groups.set([group_1, group_2])

    group_3 = ResourceGroup.objects.create(
        name="test group 3", identifier="test_group_3"
    )
    ...
    response = user_api_client.get(
        list_url + "?" + "resource_group=" + group_1.identifier
    )
    assert response.status_code == 200
    assert_response_objects(response, (reservation, reservation2))

    response = user_api_client.get(
        list_url + "?" + "resource_group=" + group_2.identifier
    )
    assert response.status_code == 200
    assert_response_objects(response, reservation2)
    ...
    response = user_api_client.get(
        list_url
        + "?"
        + "resource_group=%s,%s" % (group_1.identifier, group_2.identifier)
    )
    assert response.status_code == 200
    assert_response_objects(response, (reservation, reservation2))
...
def test_unit_filter(
    user_api_client, reservation, reservation2, resource_in_unit2, list_url
):
    reservation2.resource = resource_in_unit2
    reservation2.save()
...
@pytest.mark.django_db
def test_has_catering_order_filter(
    user_api_client,
    user,
    user2,
    resource_in_unit,
    ...
    CateringOrder.objects.create(reservation=reservation, provider=provider)
    CateringOrder.objects.create(reservation=reservation3, provider=provider)
    ...
    user.save()
    assign_perm(
        "unit:can_view_reservation_catering_orders", user, reservation3.resource.unit
    )
    response = user_api_client.get(list_url + "?has_catering_order=true")
    assert response.status_code == 200
    assert_response_objects(response, (reservation, reservation3))
...
def test_has_catering_order_field(
    ...
    user.save()
    assign_perm(
        "unit:can_view_reservation_catering_orders", user, reservation.resource.unit
    )
...
def test_manager_can_make_staff_reservation(
    ...
    """
    User with manager status on the resource should be able to make staff event
    reservations.
    """
    reservation_data["staff_event"] = True
    reservation_data["reserver_name"] = "herra huu"
...
def test_resource_filter(
    ...
    response = api_client.get(
        list_url + "?resource={},{}".format(resource_in_unit.id, resource_in_unit2.id)
    )
    assert response.status_code == 200
    assert_response_objects(response, (reservation, reservation2))
...
def test_include_resource_detail(api_client, resource_in_unit, reservation, list_url):
    ...
    assert response.status_code == 200
    assert (
        response.json()["results"][0]["resource"]["name"]["fi"] == resource_in_unit.name
    )
...
@pytest.mark.django_db
def test_reserver_info_search_filter(
    staff_api_client,
    staff_user,
    reservation,
    reservation2,
    ...
    assign_perm(
        "unit:can_view_reservation_extra_fields", staff_user, resource_in_unit2.unit
    )
    ...
    # providing query param including space assumes that first_name and last_name are
    # being provided
    elif filtering == "reserver_info_search=brendan+neutra":
        assert_response_objects(response, [reservation3])

    # martta is reserver_name in first reservation fixture, which has another unit
    # assigned, hence our staff_user should not have permissions to view reservations'
    # users' info
...
@pytest.mark.django_db
def test_reservation_normal_type_normal_user(
...
@pytest.mark.django_db
def test_reservation_block_type_normal_user(
    resource_in_unit, reservation_data, user_api_client
):
...
@pytest.mark.django_db
def test_reservation_block_type_manager(
...
@pytest.mark.django_db
def test_reservation_cannot_add_bogus_type(
    resource_in_unit, reservation_data, api_client, unit_manager_user
):
...
@pytest.mark.django_db
def test_manager_can_modify_reservations(
    resource_in_unit,
    resource_in_unit2,
    api_client,
    unit_manager_user,
...
@freeze_time("2115-04-02")
@pytest.mark.django_db
def test_manager_can_ignore_opening_hours(
    api_client, list_url, reservation_data, unit_manager_user, user
):
...
@pytest.mark.django_db
def test_manager_can_view_reservation_access_code(
    api_client, resource_in_unit, reservation, unit_manager_user
):
    resource_in_unit.access_code_type = Resource.ACCESS_CODE_TYPE_PIN6
    resource_in_unit.save()
    reservation.access_code = "123456"
...
@pytest.mark.django_db
def test_manager_can_view_reservation_extra_fields(
    api_client, resource_in_unit, reservation, unit_manager_user
):
    resource_in_unit.reservation_metadata_set = ReservationMetadataSet.objects.get(
        name="default"
...
@pytest.mark.django_db
def test_manager_can_access_reservation_comments(
    api_client, resource_in_unit, reservation, unit_manager_user
):
    ...
    assert (
        ...
        == "To be a foo or not to be a foo, that is the question"
    )


@pytest.mark.django_db
def test_manager_can_view_reservation_catering_orders(
    api_client, reservation, unit_manager_user
):
    provider = CateringProvider.objects.create(
...
@pytest.mark.django_db
def test_viewer_can_view_reservation_access_code(
    api_client, reservation, unit_viewer_user, resource_in_unit
):
    resource_in_unit.access_code_type = Resource.ACCESS_CODE_TYPE_PIN6
    resource_in_unit.save()
    reservation.access_code = "123456"
...
@pytest.mark.django_db
def test_viewer_can_view_reservation_extra_fields(
    api_client, resource_in_unit, reservation, unit_viewer_user
):
    resource_in_unit.reservation_metadata_set = ReservationMetadataSet.objects.get(
        name="default"
    )
...
@pytest.mark.django_db
def test_viewer_can_access_reservation_comments(
    api_client, reservation, unit_viewer_user
):
    reservation.comments = "To be a foo or not to be a foo, that is the question"
    reservation.save()
    detail_url = reverse("reservation-detail", kwargs={"pk": reservation.pk})
...
@pytest.mark.django_db
def test_viewer_can_view_reservation_user(
    api_client, reservation, unit_viewer_user, user
):
    detail_url = reverse("reservation-detail", kwargs={"pk": reservation.pk})
    api_client.force_authenticate(user=unit_viewer_user)
...
@pytest.mark.django_db
def test_viewer_can_modify_reservations(
    resource_in_unit,
    resource_in_unit2,
    api_client,
    unit_viewer_user,
...
@pytest.mark.django_db
def test_viewer_can_comment_reservations(
    resource_in_unit, api_client, unit_viewer_user, reservation_data, detail_url
):
    api_client.force_authenticate(user=unit_viewer_user)
    reservation_data["comments"] = "test comment"
    response = api_client.put(detail_url, data=reservation_data)
...
def test_admin_can_make_staff_reservation(
    ...
    """
    User with admin status on the resource should be able to make staff event
    reservations.
    """
    reservation_data["staff_event"] = True
    reservation_data["reserver_name"] = "herra huu"
...
def test_admin_can_create_special_type_reservation(
    ...
    """
    User with admin status on the resource should be able to make special type
    reservations.
    """
    reservation_data["type"] = Reservation.TYPE_BLOCKED
...
def test_manager_can_create_special_type_reservation(
    ...
    """
    User with manager status on the resource should be able to make special type
    reservations.
    """
    reservation_data["type"] = Reservation.TYPE_BLOCKED
...
def test_manager_can_bypass_manual_confirmation(
    ...
    """
    User with manager status on the resource should be able to bypass manual
    confirmation.
    """
...
@pytest.mark.django_db
def test_can_ignore_max_period(
    resource_with_opening_hours, user, user_api_client, list_url
):
...
@pytest.mark.django_db
def test_can_ignore_max_reservations_per_user(
    resource_with_opening_hours, user, user_api_client, list_url
):
...
@pytest.mark.django_db
def test_disallow_overlapping_reservations(
    ...
    response2 = user_api_client.post(
        list_url, reservation_data2, HTTP_ACCEPT_LANGUAGE="en"
    )
    assert response2.data["non_field_errors"][0] == expected_error
    assert response2.status_code == 400

    response3 = user_api_client.post(
        list_url, reservation_data3, HTTP_ACCEPT_LANGUAGE="en"
    )
    assert response3.data["non_field_errors"][0] == expected_error
    assert response3.status_code == 400

    response4 = user_api_client.post(
        list_url, reservation_data4, HTTP_ACCEPT_LANGUAGE="en"
    )
    assert response4.data["non_field_errors"][0] == expected_error
    assert response4.status_code == 400

    response5 = user_api_client.post(
        list_url, reservation_data5, HTTP_ACCEPT_LANGUAGE="en"
    )
    assert response5.data["non_field_errors"][0] == expected_error
    assert response5.status_code == 400

    response6 = user_api_client.post(
        list_url, reservation_data6, HTTP_ACCEPT_LANGUAGE="en"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"begin": "2115-04-04T11:00:00+02:00", "end": "2115-04-04T12:30:00+02:00"},
            False,
            400,
        ),
        (
            {"begin": "2115-04-04T08:00:00+02:00", "end": "2115-04-04T18:00:00+02:00"},
            True,
            201,
        ),
    ],
...
auth_resource_and_user_auth_combo = [
    ...
    (
        ...
        "tampere_adfs",
        201,
    ),  # Resource auth -> strong; User auth -> tampere_adfs
