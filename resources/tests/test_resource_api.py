...
@pytest.mark.django_db
def test_user_permissions_in_resource_endpoint(
    api_client, resource_in_unit, user, group
):
    """
...
def test_non_public_resource_visibility(api_client, resource_in_unit, user, staff_user):
    ...
    assert response.data["count"] == 1
    assert (
        Unit.objects.managed_by(user).values_list("id", flat=True)[0]
        == response.data["results"][0]["unit"]
    )
...
@pytest.mark.django_db
def test_filtering_by_is_favorite(
    list_url,
    api_client,
    staff_api_client,
    staff_user,
    ...
    # anonymous users don't need the filter atm, just check that using the
    # filter doesn't cause any errors
...
@pytest.mark.django_db
def test_reservable_in_advance_fields(
    api_client, resource_in_unit, test_unit, detail_url
):
    response = api_client.get(detail_url)
    assert response.status_code == 200
...
def test_resource_group_filter(
    ...
    response = api_client.get(
        "%s?resource_group=%s" % (list_url, resource_group.identifier)
    )
    assert response.status_code == 200
    ...
    # multiple groups
    response = api_client.get(
        "%s?resource_group=%s,%s"
        % (list_url, resource_group.identifier, resource_group2.identifier)
    )
    assert response.status_code == 200
    assert set(r["id"] for r in response.data["results"]) == {
        resource_in_unit.id,
...
def test_reservation_extra_fields(api_client, resource_in_unit):
    ...
    supported_fields = set(
        default_set.supported_fields.values_list("field_name", flat=True)
    )
    assert set(response.data["supported_reservation_extra_fields"]) == supported_fields

    required_fields = set(
        default_set.required_fields.values_list("field_name", flat=True)
    )
...
def test_resource_type_filter(
    api_client, resource_in_unit, resource_in_unit2, resource_in_unit3, list_url
):
    ...
    assert response.status_code == 200
    assert {resource["id"] for resource in response.data["results"]} == {
        resource_in_unit.id
    }
...
@pytest.mark.django_db
def test_resource_equipment_filter(
    api_client,
    resource_in_unit,
    resource_in_unit2,
    resource_in_unit3,
    ...
    response = api_client.get(
        list_url + "?equipment=%s,%s" % (equipment_1.id, equipment_2.id)
    )
    assert response.status_code == 200
    assert {resource["id"] for resource in response.data["results"]} == {
        resource_in_unit.id,
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            {
                "available_between": "2115-04-08T08:00:00+02:00,2115-04-08T10:00:00+02:00"  # noqa
            },
            [0, 1],
        ),
        (
            {
                "available_between": "2115-04-08T08:00:00+02:00,2115-04-08T10:00:01+02:00"  # noqa
            },
            [1],
        ),
        (
            {
                "available_between": "2115-04-08T10:59:59+02:00,2115-04-08T12:00:00+02:00"  # noqa
            },
            [1],
        ),
        (
            {
                "available_between": "2115-04-08T10:59:59+02:00,2115-04-08T12:00:01+02:00"  # noqa
            },
            [],
        ),
        (
            {
                "available_between": "2115-04-08T13:00:00+02:00,2115-04-08T18:00:00+02:00"  # noqa
            },
            [0, 1],
        ),
    ),
...
@pytest.mark.django_db
def test_resource_available_between_filter_reservations(
    user_api_client,
    list_url,
    user,
    resource_in_unit,
    ...
    # set resources open practically the whole so that opening hours don't
    # intervene in this test
    for resource in resources:
        p1 = Period.objects.create(
            start=datetime.date(2115, 4, 1),
    ...
    assert response.status_code == 200
    assert_response_objects(
        response, [resources[index] for index in expected_resource_indexes]
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            {
                "available_between": "2115-04-08T06:00:00+02:00,2115-04-08T07:00:00+02:00"  # noqa
            },
            [],
        ),
        (
            {
                "available_between": "2115-04-08T07:59:59+02:00,2115-04-08T16:00:00+02:00"  # noqa
            },
            [],
        ),
        (
            {
                "available_between": "2115-04-08T08:00:00+02:00,2115-04-08T16:00:00+02:00"  # noqa
            },
            [0],
        ),
        (
            {
                "available_between": "2115-04-08T08:00:00+02:00,2115-04-08T16:00:01+02:00"  # noqa
            },
            [],
        ),
        (
            {
                "available_between": "2115-04-08T12:00:00+02:00,2115-04-08T14:00:00+02:00"  # noqa
            },
            [0, 1],
        ),
        (
            {
                "available_between": "2115-04-14T12:00:00+02:00,2115-04-14T14:00:00+02:00"  # noqa
            },
            [0],
        ),
    ),
...
@pytest.mark.django_db
def test_resource_available_between_filter_opening_hours(
    user_api_client,
    list_url,
    resource_in_unit,
    resource_in_unit2,
...
@pytest.mark.django_db
def test_resource_available_between_filter_constraints(
    user_api_client, list_url, resource_in_unit
):
    response = user_api_client.get(
        list_url, {"available_between": "2115-04-08T00:00:00+02:00"}
    )
    assert response.status_code == 400
    assert "available_between takes two or three comma-separated values." in str(
        response.data
    )

    response = user_api_client.get(
        list_url, {"available_between": "2115-04-08T00:00:00+02:00,100,100,100"}
    )
    assert response.status_code == 400
    assert "available_between takes two or three comma-separated values." in str(
        response.data
    ...
    response = user_api_client.get(
        list_url,
        {
            "available_between": "2115-04-08T00:00:00+02:00,2115-04-08T00:00:00+02:00,xyz"  # noqa
        },
    )
    assert response.status_code == 400
...
@pytest.mark.django_db
def test_resource_available_between_considers_inactive_reservations(
    user_api_client, user, list_url, resource_in_unit
):
    p1 = Period.objects.create(
        start=datetime.date(2115, 4, 1),
        end=datetime.date(2115, 4, 30),
    ...
    params = {
        "available_between": "2115-04-08T08:00:00+02:00,2115-04-08T16:00:00+02:00"
    }
...
@pytest.mark.django_db
def test_available_between_with_period(
    list_url,
    resource_in_unit,
    resource_in_unit2,
    resource_in_unit3,
    ...
    params = {
        "available_between": "2115-04-08T{}:00+02:00,2115-04-08T{}:00+02:00,{}".format(
            start, end, period
        )
    }
    expected_resources = [
        r for i, r in enumerate([resource_in_unit, resource_in_unit2]) if i in expected
    ]
    response = user_api_client.get(list_url, params)
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_filtering_by_municipality(
    list_url, api_client, resource_in_unit, test_unit, test_municipality
):
...
def test_order_by_filter(list_url, api_client, resource_in_unit, resource_in_unit2):
    ...
    (
        resource_in_unit2.name_fi,
        resource_in_unit2.name_en,
        resource_in_unit2.name_sv,
    ) = "bbb"
    resource_in_unit2.save()
    ...
    # test unit_name_fi
    (
        resource_in_unit.unit.name_fi,
        resource_in_unit.unit.name_en,
        resource_in_unit.unit.name_sv,
    ) = "aaa"
    resource_in_unit.unit.save()

    (
        resource_in_unit2.unit.name_fi,
        resource_in_unit2.unit.name_en,
        resource_in_unit2.unit.name_sv,
    ) = "bbb"
...
@pytest.mark.django_db
def test_order_by_accessibility_no_viewpoint(
    list_url,
    api_client,
    resource_with_accessibility_data,
    resource_with_accessibility_data2,
):
    """Ordering by accessibility requires that the desired accessibility
    viewpoint for ordering is provided
    """
...
@pytest.mark.django_db
def test_order_by_accessibility(
    list_url,
    api_client,
    resource_with_accessibility_data,
    resource_with_accessibility_data2,
    ...
    """
    response = api_client.get(
        "{}?order_by=-accessibility&accessibility_viewpoint={}".format(
    ...
    assert_response_objects(
        response, [resource_with_accessibility_data, resource_with_accessibility_data2]
    )
    assert (
        response.data["results"][0]["name"]["fi"]
        == resource_with_accessibility_data.name_fi
    )
    assert (
        response.data["results"][1]["name"]["fi"]
        == resource_with_accessibility_data2.name_fi
    )
...
@pytest.mark.django_db
def test_order_by_accessibility_inaccessible_unit(
    list_url,
    api_client,
    resource_with_accessibility_data,
    ...
    assert_response_objects(
        response, [resource_with_accessibility_data, resource_with_accessibility_data3]
    )
    assert (
        response.data["results"][0]["name"]["fi"]
        == resource_with_accessibility_data.name_fi
    ...
    assert (
        response.data["results"][1]["name"]["fi"]
        == resource_with_accessibility_data3.name_fi
    )


@pytest.mark.django_db
def test_resource_with_accessibility_data(
    api_client, resource_with_accessibility_data, detail_url
):
    """Resource endpoint should include accessibility data
