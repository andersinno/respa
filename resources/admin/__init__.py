...
from .base import (
    CommonExcludeMixin,
    ExtraReadonlyFieldsOnUpdateMixin,
...
class CustomUserManage(forms.Form):
    ...
    user = EmailAndUsernameChoiceField(
        ...
        .objects.filter(Q(auth_token__isnull=False) | Q(is_staff=True))
        .distinct()
...
class ResourceEquipmentInline(
    PopulateCreatedAndModifiedMixin, CommonExcludeMixin, TranslationStackedInline
):
...
class ResourceGroupInline(
    PopulateCreatedAndModifiedMixin, CommonExcludeMixin, admin.TabularInline
):
...
class PeriodTemplateListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (
            ("is_template", _("Templates only")),
            ("is_not_template", _("Non-templates only")),
        )

    def queryset(self, request, queryset):
        selected_dropdown_filter = self.value()
        all_periods = Period.objects.all()

        if selected_dropdown_filter == "is_template":
            return all_periods.filter(is_template=True)
        if selected_dropdown_filter == "is_not_template":
            return all_periods.exclude(is_template=True)
        return all_periods
...
class PeriodAdmin(
    ...
    list_filter = (PeriodTemplateListFilter,)
    inlines = [
        DayInline,
    ]
    ...
    def get_form(self, request, obj=None, change=False, **kwargs):
        ...
        form.base_fields["template_src"].queryset = Period.objects.filter(
            is_template=True
        )
        return form
    ...
    def tprek_import(self, request):
        ...
        with redirect_stdout(out):
            try:
                ...
                context[
                    "command_output"
                ] = "Running import script caused the following exception: {0}".format(
                    str(e)
                )
...
class LimitAuthorizedToStaff(admin.ModelAdmin):
    def get_field_queryset(self, db, db_field, request):
        ...
        if db_field.name == "authorized":
            return qs.filter(is_staff=True).order_by("last_name", "first_name", "email")
        return qs
...
class UnitGroupAuthorizationAdmin(
    _CommonMixin, LimitAuthorizedToStaff, admin.ModelAdmin
):
    list_display = ["id", "subject", "level", "authorized"]


class ResourceImageAdmin(
    PopulateCreatedAndModifiedMixin,
    CommonExcludeMixin,
    ImageCroppingMixin,
    TranslationAdmin,
...
class EquipmentAdmin(
    ...
    inlines = (EquipmentAliasInline,)


class ResourceEquipmentAdmin(
    PopulateCreatedAndModifiedMixin, CommonExcludeMixin, TranslationAdmin
):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        if db_field.name == "field_name":
            ...
            kwargs["widget"].choices = [
                ...
                if c[0]
                not in ReservationMetadataField.objects.values_list(
